...
export function InstanceSettings() {
  ...
  const toggleMutation = useMutation({
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("heartbeats.failedToUpdateHeartbeat"));
    },
  });
  ...
  const disableAllMutation = useMutation({
    mutationFn: async (agentRows: InstanceSchedulerHeartbeatAgent[]) => {
      ...
      if (failures.length > 0) {
        ...
        const detail = firstError instanceof Error ? firstError.message : t("heartbeats.unknownError");
        throw new Error(
          failures.length === 1
            ? t("heartbeats.failedToDisableOne", { detail })
            : t("heartbeats.failedToDisableMany", { failed: failures.length, enabled: enabled.length, detail }),
        );
      }
      return enabled;
    },
    ...
    onError: (error) => {
      setActionError(error instanceof Error ? error.message : t("heartbeats.failedToDisableAll"));
    },
  });
  ...
  return (
    <div className="max-w-5xl space-y-6">
      ...
      <div className="flex items-center gap-4 text-sm text-muted-foreground">
        <span><span className="font-semibold text-foreground">{activeCount}</span> {t("common.active").toLowerCase()}</span>
        <span><span className="font-semibold text-foreground">{disabledCount}</span> {t("heartbeats.disabled")}</span>
        <span><span className="font-semibold text-foreground">{grouped.length}</span> {t("heartbeats.companies", { count: grouped.length })}</span>
        {anyEnabled && (
          <Button
            variant="destructive"
            size="sm"
            ...
            onClick={() => {
              if (!window.confirm(t("heartbeats.disableAllConfirm", { count: enabledCount }))) {
                return;
              }
      ...
      {agents.length === 0 ? (
        <EmptyState
          icon={Clock3}
          message={t("heartbeats.noHeartbeats")}
        />
      ) : (
                      ...
                      >
                        ...
                        <span className="ml-auto flex items-center gap-1.5 shrink-0">
                          <Link
                            ...
                            title={t("heartbeats.fullAgentConfig")}
                          >
                          ...
                          >
                            {saving ? "..." : agent.heartbeatEnabled ? t("heartbeats.disableTimer") : t("heartbeats.enableTimer")}
                          </Button>
                        </span>
                      </div>
                    );
