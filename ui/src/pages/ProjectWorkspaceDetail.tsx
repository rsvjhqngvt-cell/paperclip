...
const SOURCE_TYPE_VALUES: Array<{ value: ProjectWorkspaceSourceType; labelKey: string; descKey: string }> = [
  { value: "local_path", labelKey: "projectWorkspace.sourceLocalGit", descKey: "projectWorkspace.sourceLocalGitDesc" },
  { value: "non_git_path", labelKey: "projectWorkspace.sourceNonGit", descKey: "projectWorkspace.sourceNonGitDesc" },
  { value: "git_repo", labelKey: "projectWorkspace.sourceGitRepo", descKey: "projectWorkspace.sourceGitRepoDesc" },
  { value: "remote_managed", labelKey: "projectWorkspace.sourceRemoteManaged", descKey: "projectWorkspace.sourceRemoteManagedDesc" },
];

const VISIBILITY_VALUES: Array<{ value: ProjectWorkspaceVisibility; labelKey: string }> = [
  { value: "default", labelKey: "projectWorkspace.visibilityDefault" },
  { value: "advanced", labelKey: "projectWorkspace.visibilityAdvanced" },
];
...
function parseRuntimeConfigJson(value: string) {
  ...
  try {
    ...
    if (!parsed || typeof parsed !== "object" || Array.isArray(parsed)) {
      return {
        ok: false as const,
        error: "projectWorkspace.errRuntimeJsonObject",
      };
    }
    ...
    return {
      ok: false as const,
      error: error instanceof Error ? error.message : "projectWorkspace.errInvalidJson",
    };
  }
}
...
function validateWorkspaceForm(form: WorkspaceFormState) {
  ...
  if (form.sourceType === "remote_managed") {
    if (!remoteWorkspaceRef && !repoUrl) {
      return "projectWorkspace.errRemoteRequires";
    }
  } else if (!cwd && !repoUrl) {
    return "projectWorkspace.errAtLeastOnePath";
  }

  if (cwd && (form.sourceType === "local_path" || form.sourceType === "non_git_path") && !isAbsolutePath(cwd)) {
    return "projectWorkspace.errLocalPathAbsolute";
  }
  ...
  if (repoUrl) {
    try {
      ...
      return "projectWorkspace.errRepoUrlInvalid";
    }
  }
...
export function ProjectWorkspaceDetail() {
  ...
  useEffect(() => {
    ...
    setBreadcrumbs([
      ...
      { label: t("projects.workspacesBreadcrumb"), href: `/projects/${canonicalProjectRef}/workspaces` },
  ...
  const updateWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("projectWorkspace.errFailedToSave"));
    },
  });
  ...
  const setPrimaryWorkspace = useMutation({
    ...
    onError: (error) => {
      setErrorMessage(error instanceof Error ? error.message : t("projectWorkspace.errFailedToUpdate"));
    },
  });
  ...
  const controlRuntimeServices = useMutation({
    ...
    onSuccess: (result, action) => {
      ...
      setRuntimeActionMessage(
        action === "stop"
          ? t("projectWorkspace.runtimeStopped")
          : action === "restart"
            ? t("projectWorkspace.runtimeRestarted")
            : t("projectWorkspace.runtimeStarted"),
      );
    },
    onError: (error) => {
      setRuntimeActionMessage(null);
      setErrorMessage(error instanceof Error ? error.message : t("projectWorkspace.errFailedToControl"));
    },
  });
  ...
  const saveChanges = () => {
    ...
    try {
      ...
      const raw = error instanceof Error ? error.message : "projectWorkspace.errFailedToSave";
      setErrorMessage(t(raw, { defaultValue: raw }));
      return;
    }
  ...
  const sourceTypeDescription = SOURCE_TYPE_VALUES.find((option) => option.value === form.sourceType);
  const sourceTypeDescriptionText = sourceTypeDescription ? t(sourceTypeDescription.descKey) : null;
    ...
    <div className="mx-auto max-w-5xl space-y-6">
      <div className="flex flex-wrap items-center gap-3">
        ...
        <div className="inline-flex items-center rounded-full border border-border bg-background px-2.5 py-1 text-xs text-muted-foreground">
          {workspace.isPrimary ? t("projectWorkspace.primaryBadge") : t("projectWorkspace.secondaryBadge")}
        </div>
      </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
              ...
              {!workspace.isPrimary ? (
                ...
                <div className="inline-flex items-center gap-2 rounded-xl border border-emerald-500/25 bg-emerald-500/10 px-3 py-2 text-sm text-emerald-700 dark:text-emerald-300 sm:max-w-sm">
                  ...
                  {t("projectWorkspace.primaryNote")}
                </div>
              )}
            </div>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              <Field label={t("projectWorkspace.workspaceName")}>
                <input
                  ...
                  placeholder={t("projects.workspaceNamePlaceholder")}
                />
              </Field>
            ...
            <div className="mt-4 grid gap-4">
              <Field label={t("projectWorkspace.sourceType")} hint={sourceTypeDescriptionText ?? undefined}>
                <select
                  className="w-full rounded-lg border border-border bg-background px-3 py-2 text-sm outline-none"
              ...
              <div className="grid gap-4 md:grid-cols-[minmax(0,1fr)_auto]">
                <Field label={t("projectWorkspace.localPath")}>
                  <input
                    ...
                    placeholder={t("projectWorkspace.localPathPlaceholder")}
                  />
                </Field>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("projectWorkspace.repoUrl")}>
                  <input
                    ...
                    placeholder={t("projectWorkspace.repoUrlPlaceholder")}
                  />
                </Field>
                <Field label={t("projectWorkspace.repoRef")}>
                  <input
                    ...
                    placeholder={t("projectWorkspace.repoRefPlaceholder")}
                  />
                </Field>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("projectWorkspace.defaultRef")}>
                  <input
                    ...
                    placeholder={t("projectWorkspace.defaultRefPlaceholder")}
                  />
                </Field>
                <Field label={t("projectWorkspace.sharedWorkspaceKey")}>
                  <input
                    ...
                    placeholder={t("projectWorkspace.sharedWorkspaceKeyPlaceholder")}
                  />
                </Field>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("projectWorkspace.remoteProvider")}>
                  <input
                    ...
                    placeholder={t("projectWorkspace.remoteProviderPlaceholder")}
                  />
                </Field>
                <Field label={t("projectWorkspace.remoteWorkspaceRef")}>
                  <input
                    ...
                    placeholder={t("projectWorkspace.remoteWorkspaceRefPlaceholder")}
                  />
                </Field>
              </div>
              ...
              <div className="grid gap-4 md:grid-cols-2">
                <Field label={t("projectWorkspace.setupCommand")} hint={t("projectWorkspace.setupCommandHint")}>
                  <textarea
                    ...
                    placeholder={t("projectWorkspace.setupCommandPlaceholder")}
                  />
                </Field>
                <Field label={t("projectWorkspace.cleanupCommand")} hint={t("projectWorkspace.cleanupCommandHint")}>
                  <textarea
                    ...
                    placeholder={t("projectWorkspace.cleanupCommandPlaceholder")}
                  />
                </Field>
              </div>
        ...
        <div className="space-y-6">
          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="space-y-1">
              <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("projects.workspaceFacts")}</div>
            ...
            <DetailRow label={t("projectWorkspace.localPathLabel")}>
              <span className="break-all font-mono text-xs">{workspace.cwd ?? t("common.none")}</span>
            </DetailRow>
            <DetailRow label={t("projectWorkspace.repo")}>
            ...
            <DetailRow label={t("projectWorkspace.defaultRefLabel")}>{workspace.defaultRef ?? t("common.none")}</DetailRow>
            <DetailRow label={t("projectWorkspace.updated")}>{new Date(workspace.updatedAt).toLocaleString()}</DetailRow>
          </div>

          <div className="rounded-2xl border border-border bg-card p-5">
            <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
              <div className="space-y-1">
                <div className="text-xs font-medium uppercase tracking-[0.16em] text-muted-foreground">{t("projects.runtimeServices")}</div>
                <h2 className="text-lg font-semibold">{t("projects.attachedServices")}</h2>
                <p className="text-sm text-muted-foreground">
                  {t("projectWorkspace.sharedServicesDesc")}
                </p>
              </div>
            ...
            {workspace.runtimeServices && workspace.runtimeServices.length > 0 ? (
                    ...
                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                      <div className="space-y-1">
                        ...
                        <div className="space-y-1 text-xs text-muted-foreground">
                          ...
                          {service.port ? <div>{t("projectWorkspace.portValue", { port: service.port })}</div> : null}
              ...
              <p className="text-sm text-muted-foreground">
                {workspace.runtimeConfig?.workspaceRuntime
                  ? t("projectWorkspace.noRuntimeServicesRunning")
                  : t("projectWorkspace.noRuntimeConfig")}
              </p>
            )}
          </div>
        </div>
