...
export function OnboardingWizard() {
  ...
  async function runAdapterEnvironmentTest(
    ...
    if (!createdCompanyId) {
      setAdapterEnvError(
        t("onboarding.createOrSelectCompanyBeforeTesting")
      );
      return null;
    }
    ...
    try {
      if (adapterType === "opencode_local") {
        ...
        if (adapterModelsLoading || adapterModelsFetching) {
          setError(
            t("onboarding.openCodeModelsStillLoading")
          );
          return;
        }
  ...
  async function handleUnsetAnthropicApiKey() {
    ...
    try {
      ...
      if (result?.status === "fail") {
        setError(
          t("onboarding.retriedWithUnsetAnthropicStillFailing")
        );
      }
    } catch (err) {
      setError(
        err instanceof Error
          ? err.message
          : t("onboarding.failedToUnsetAnthropicApiKey")
      );
    } finally {
      setUnsetAnthropicLoading(false);
    }
                ...
                <div className="space-y-5">
                  ...
                  <div className="group">
                    ...
                    <textarea
                      ...
                      placeholder={t("onboarding.companyGoalPlaceholder")}
                      value={companyGoal}
              ...
              {step === 2 && (
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <input
                      className="w-full rounded-md border border-border bg-transparent px-3 py-2 text-sm outline-none focus:ring-1 focus:ring-ring placeholder:text-muted-foreground/50"
                      placeholder={t("onboarding.ceoPlaceholder")}
                      value={agentName}
                      ...
                      {[
                        {
                          ...
                          label: t("onboarding.adapterClaudeCode"),
                          icon: Sparkles,
                          desc: t("onboarding.adapterClaudeDesc"),
                          recommended: true
                        },
                        {
                          value: "codex_local" as const,
                          label: t("onboarding.adapterCodex"),
                          icon: Code,
                          desc: t("onboarding.adapterCodexDesc"),
                          recommended: true
                        }
                      ].map((opt) => (
                        <button
                        ...
                        {[
                          {
                            ...
                            label: t("onboarding.adapterGeminiCli"),
                            icon: Gem,
                            desc: t("onboarding.adapterGeminiDesc")
                          },
                          {
                            value: "opencode_local" as const,
                            label: t("onboarding.adapterOpenCode"),
                            icon: OpenCodeLogoIcon,
                            desc: t("onboarding.adapterMultiProviderDesc")
                          },
                          {
                            value: "pi_local" as const,
                            label: t("onboarding.adapterPi"),
                            icon: Terminal,
                            desc: t("onboarding.adapterPiDesc")
                          },
                          {
                            value: "cursor" as const,
                            label: t("onboarding.adapterCursor"),
                            icon: MousePointer2,
                            desc: t("onboarding.adapterCursorDesc")
                          },
                          {
                            value: "hermes_local" as const,
                            label: t("onboarding.adapterHermes"),
                            icon: HermesIcon,
                            desc: t("onboarding.adapterMultiProviderDesc")
                          },
                          {
                            value: "openclaw_gateway" as const,
                            label: t("onboarding.adapterOpenClawGateway"),
                            icon: Bot,
                            desc: t("onboarding.adapterOpenClawGatewayDesc"),
                            comingSoon: true,
                            disabledLabel: t("onboarding.configureOpenClawInApp")
                          }
                        ].map((opt) => (
                          <button
                            key={opt.value}
                      ...
                      <div className="flex items-center justify-between gap-2">
                        <div>
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            {t("onboarding.adapterEnvironmentCheckDesc")}
                          </p>
                        </div>
                        <Button
                          size="sm"
                      ...
                      {shouldSuggestUnsetAnthropicApiKey && (
                        <div className="rounded-md border border-amber-300/60 bg-amber-50/40 px-2.5 py-2 space-y-2">
                          <p className="text-[11px] text-amber-900/90 leading-relaxed">
                            {t("onboarding.claudeFailedPrefix")}{" "}
                            <span className="font-mono">ANTHROPIC_API_KEY</span>{" "}
                            {t("onboarding.claudeFailedSuffix")}
                          </p>
                          <Button
                            size="sm"
                            variant="outline"
                      ...
                      {adapterEnvResult && adapterEnvResult.status === "fail" && (
                        <div className="rounded-md border border-border/70 bg-muted/20 px-2.5 py-2 text-[11px] space-y-1.5">
                          ...
                          <p className="text-muted-foreground font-mono break-all">
                            {adapterType === "cursor"
                              ? `${effectiveAdapterCommand} -p --mode ask --output-format json \"${t("onboarding.respondWithHello")}"`
                              : adapterType === "codex_local"
                              ? `${effectiveAdapterCommand} exec --json -`
                              : adapterType === "gemini_local"
                                ? `${effectiveAdapterCommand} --output-format json "${t("onboarding.respondWithHello")}"`
                              : adapterType === "opencode_local"
                                ? `${effectiveAdapterCommand} run --format json "${t("onboarding.respondWithHello")}"`
                ...
                <div className="space-y-5">
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      placeholder={t("onboarding.taskDetailPlaceholder")}
          ...
          >
            ...
            {check.hint && (
              <span className="block opacity-90 break-words">
                {t("onboarding.hintLabel")}: {check.hint}
              </span>
            )}
          </div>
        ))}
